...
async function sendSolToHouse(lamports){
  ...
  // Use signTransaction + sendRawTransaction to avoid Phantom blocking
  const signed = await phantom.signTransaction(transaction);
  const rawTx = signed.serialize();
  const sendResp = await fetch(HELIUS_RPC_BET, {
    method: 'POST',
    headers: {'Content-Type':'application/json'},
    body: JSON.stringify({
      jsonrpc:'2.0', id:1,
      method:'sendTransaction',
      params:[btoa(String.fromCharCode(...new Uint8Array(rawTx))), {encoding:'base64', preflightCommitment:'confirmed'}]
    })
  });
