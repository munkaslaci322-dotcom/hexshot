...
let waitingTimer  = null; // auto-refund timer
...
function onServerMsg(msg){
  ...
  if (msg.type==='room_created'){
    ...
    // Start 5-min auto-refund countdown
  ...
  if (msg.type==='game_start'){
    // Cancel waiting timer — opponent joined
    clearWaitingTimer();
    myPlayerIdx = (msg.players||[]).findIndex(p=>p.walletAddress===phantomPublicKey);
    if (myPlayerIdx<0) myPlayerIdx=0;
    myRoomId=msg.roomId; gameBet=msg.bet; gameActive=true;
    myScore=oppScore=0;
    showScreen('game-screen');
    refreshScores(); startTimer();
  ...
  if (msg.type==='shot_result'){
    ...
    refreshScores(); return;
  }

  if (msg.type==='timer'){ gameTimer=msg.timeLeft; renderTimer(); return; }
  ...
  if (msg.type==='refunded'){
    clearWaitingTimer();
    const amt = parseFloat(msg.amount||0).toFixed(3);
    showToast('↩️ '+amt+' SOL refunded (no opponent found)', 6000);
    resetBetBtn();
...
function renderLiveRooms(rooms){
  ...
  if (rooms.length === 0){
    container.innerHTML = `
      <div style="text-align:center;padding:40px 20px;color:var(--muted)">
        ...
        <div style="font-size:0.85rem;font-weight:600;margin-bottom:6px">No open rooms</div>
  ...
  container.innerHTML = rooms.map(r => `
    <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;
         background:rgba(20,20,40,0.7);border:1px solid var(--border);border-radius:12px;
      ...
      <div style="flex:1">
        <div style="font-size:0.82rem;font-weight:700;color:#fff;margin-bottom:2px">
          Room #${r.roomId}
        </div>
        <div style="font-size:0.68rem;color:var(--muted)">
          ${shortAddr(r.walletAddress||'???')} · ◎ ${parseFloat(r.bet||0).toFixed(3)} SOL
        </div>
      </div>
      <div style="text-align:right">
        <div style="font-family:'Space Mono',monospace;font-size:0.85rem;font-weight:700;
             color:var(--yellow);margin-bottom:4px">◎ ${parseFloat(r.bet||0).toFixed(3)}</div>
        <button onclick="joinSpecificRoom('${r.roomId}', ${r.bet})"
          style="padding:6px 14px;background:linear-gradient(135deg,#6d28d9,#a855f7);
...
async function joinSpecificRoom(roomId, bet){
  ...
  if (bet > walletBalance){ showToast('❌ Not enough SOL! Need '+bet.toFixed(3)+' SOL'); return; }
  gameBet = bet;
  ...
  try{
    ...
    connectWS();
    const go = ()=>wsSend({type:'join_room',roomId:roomId,walletAddress:phantomPublicKey,txSig:sig});
...
function startWaitingTimer(roomId){
  ...
  waitingTimer = setInterval(()=>{
    ...
    if(secondsLeft <= 0){
      ...
      showToast('⏰ No opponent found — requesting refund...', 5000);
...
async function placeBet(){
  ...
  if(!walletConnected){openWalletModal();return;}
  if(bet>walletBalance){showToast('❌ Not enough SOL! Balance: '+walletBalance.toFixed(3)+' SOL');return;}
...
async function showJoinModal(){
  ...
  if(!code) return;
  await joinSpecificRoom(code.trim(), gameBet);
}
...
function initChat(){
  ...
  for(let i=0;i<6;i++) setTimeout(()=>{
    addChatMsg(FAKE_USERS[Math.floor(Math.random()*FAKE_USERS.length)],FAKE_MSGS[Math.floor(Math.random()*FAKE_MSGS.length)],FAKE_COLORS[Math.floor(Math.random()*FAKE_COLORS.length)]);
  },i*500);
  setInterval(()=>{
    addChatMsg(FAKE_USERS[Math.floor(Math.random()*FAKE_USERS.length)],FAKE_MSGS[Math.floor(Math.random()*FAKE_MSGS.length)],FAKE_COLORS[Math.floor(Math.random()*FAKE_COLORS.length)]);
...
function showToast(msg,dur=3000){
  ...
  if(!c){c=document.createElement('div');c.id='toast-container';c.style.cssText='position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;flex-direction:column;gap:8px;pointer-events:none';document.body.appendChild(c);}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Start polling for live rooms after WS connects
  setTimeout(startRoomPolling, 1000);
});
