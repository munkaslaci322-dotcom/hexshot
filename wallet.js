...
async function fetchSolPrice(){
  // Use Helius to get SOL price (avoids CORS issues)
  try {
    const r = await fetch('https://api.binance.com/api/v3/ticker/price?symbol=SOLUSDT');
    const d = await r.json();
    if (d?.price){ SOL_USD = parseFloat(d.price); updateWalletDisplay(); return; }
  } catch {}
  try {
...
async function _finishConnect(phantom, pubKey){
  ...
  if (typeof wsSend === 'function') wsSend({type:'wallet_connect', walletAddress: pubKey});
  closeWalletModal();
...
function disconnectWallet(){
  ...
  // Notify server wallet disconnected
  if (typeof wsSend === 'function' && typeof phantomPublicKey !== 'undefined')
