...
let phantomPublicKey  = null;   // STRING — the user's OWN wallet
let walletBalance     = 0;
let SOL_USD           = 150;
...
function getPhantom(){
  // always return the provider — never auto-select an account here
...
async function connectPhantom(){
  ...
  try {
    // force popup — user picks account themselves
    const resp   = await phantom.connect({ onlyIfTrusted: false });
    const pubKey = resp.publicKey.toString();

    // GUARD: never allow house wallet to be "the user"
    if (pubKey === HOUSE_WALLET){
      showToast('❌ Switch to your personal wallet in Phantom first!');
      if (desc) desc.textContent = '❌ Wrong account selected';
    ...
    // react to account switch inside Phantom
