...
// ── Balance — use Phantom's own connection ────
async function fetchBalance(pubkeyStr){
  // Best method: use Phantom's trusted connection directly
  try {
    const phantom = getPhantom();
    if (phantom && typeof solanaWeb3 !== 'undefined') {
      // Try multiple public RPC endpoints
      const endpoints = [
        'https://mainnet.helius-rpc.com/?api-key=ad3029b1-970c-4f66-a68d-58301f7c0a3a',
        'https://mainnet.helius-rpc.com/?api-key=ad3029b1-970c-4f66-a68d-58301f7c0a3a',
        'https://api.mainnet-beta.solana.com',
      ];
      for (const ep of endpoints) {
        try {
          const conn = new solanaWeb3.Connection(ep, 'confirmed');
          const pk   = new solanaWeb3.PublicKey(pubkeyStr);
