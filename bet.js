...
function onServerMsg(msg){
  ...
  if (msg.type === 'game_chat'){
    ...
    if(list){
      ...
      const col = msg.isMe ? '#a855f7' : '#22d3ee';
      d.innerHTML = '<div class="chat-avatar" style="background:'+col+'">'+( msg.user||'?').slice(0,2).toUpperCase()+'</div><div class="chat-msg-body"><div class="chat-msg-name" style="color:'+col+'">'+( msg.user||'Player')+'</div><div class="chat-msg-text">'+msg.text+'</div></div>';
      list.appendChild(d);
...
function sendChatMsg(){
  ...
  if(wsReady) wsSend({type:'game_chat', user, text:text.slice(0,100), roomId:myRoomId});
  // Don't show locally - server will echo back with isMe:true
  inp.value = '';
}
