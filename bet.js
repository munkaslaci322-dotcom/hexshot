...
function onServerMsg(msg){
  ...
  if (msg.type === 'chat'){
    // Only show if not from ourselves (we already showed it locally)
    if (msg.user !== (phantomPublicKey ? shortAddr(phantomPublicKey) : 'Guest')){
      addChatMsg(msg.user || 'Player', msg.text || '', msg.color || '#64748b');
    }
    return;
  }
...
function startRoomPolling(){
  const reqRooms = () => { if (wsReady) wsSend({type:'get_rooms'}); };
  const reqOnline = () => { if (wsReady) wsSend({type:'get_online'}); };
  reqRooms(); reqOnline();
  setInterval(reqRooms, 4000);
  setInterval(reqOnline, 8000);
}
...
async function placeBet(){
  ...
  try {
    // Make sure WS is connected BEFORE taking payment
    if (!wsReady) {
      connectWS();
      showToast('⏳ Connecting to server...');
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('Server connection timeout')), 15000);
        const check = setInterval(() => {
          if (wsReady) { clearInterval(check); clearTimeout(timeout); resolve(); }
        }, 200);
      });
    }
    showToast('👻 Approve in Phantom...');
...
function sendGlobalMsg(){
  ...
  if (!text) return;
  // Spam protection: 2 second cooldown
  const now = Date.now();
  if (now - _lastChatTime < 2000){
    showToast('⏳ Please wait before sending again', 1500);
    return;
  }
  _lastChatTime = now;
  const user = phantomPublicKey ? shortAddr(phantomPublicKey) : 'Guest';
  if (wsReady) {
    wsSend({type:'chat', user, text: text.slice(0,120), color:'#a855f7'});
  }
...
function initChat(){
  ...
  // No fake messages - only real player messages
  // Request online count from server
  if (wsReady) wsSend({type:'get_online'});
}
...
function sendChatMsg(){
  ...
  const text = inp.value.trim();
  if (!text) return;
  const user = typeof phantomPublicKey !== 'undefined' && phantomPublicKey ? shortAddr(phantomPublicKey) : 'Player';
  const list = document.getElementById('game-messages');
  if (list){
    const d = document.createElement('div');
    d.className = 'chat-msg';
    d.innerHTML = '<div class="chat-avatar" style="background:#a855f7">' + user.slice(0,2).toUpperCase() + '</div><div class="chat-msg-body"><div class="chat-msg-name" style="color:#a855f7">' + user + '</div><div class="chat-msg-text">' + text + '</div></div>';
    list.appendChild(d);
    list.scrollTop = list.scrollHeight;
  }
  inp.value = '';
}
function sendGameMsg(){ sendChatMsg(); }
