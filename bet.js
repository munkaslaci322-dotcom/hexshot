...
async function placeBet(){
  ...
  if (!walletConnected){ openWalletModal(); return; }
  const SOL_FEE = 0.000010; // ~2x Solana tx fee buffer
  if (walletBalance < SOL_FEE){
    showToast('❌ Not enough SOL for transaction fees! You need at least ◎' + SOL_FEE.toFixed(5) + ' SOL in your wallet.', 6000);
    return;
  }
  if (bet + SOL_FEE > walletBalance){
    const maxBet = Math.max(0, walletBalance - SOL_FEE);
    showToast('❌ Not enough SOL! Your balance: ◎' + walletBalance.toFixed(4) + ' (max bet: ◎' + maxBet.toFixed(4) + ')', 6000);
    return;
  }
  gameBet = bet;
  ...
  try {
    ...
    // Parse error message — catch "blocked", insufficient funds, user rejection etc.
    ...
    if (err.code === 4001 || msg.includes('rejected') || msg.includes('User rejected')){
      ...
      showToast('❌ Network error — please try again in a moment.', 4000);
    } else {
...
async function _doJoinWithBet(roomId, bet){
  const SOL_FEE = 0.000010;
  if (walletBalance < SOL_FEE){
    showToast('❌ Not enough SOL for transaction fees! Need at least ◎' + SOL_FEE.toFixed(5), 6000);
    return;
  }
  if (bet + SOL_FEE > walletBalance){
    showToast('❌ Not enough SOL! This room requires ◎' + bet.toFixed(3) + ' but you only have ◎' + walletBalance.toFixed(4), 6000);
    return;
  }
  gameBet = bet;
