...
function connectWS(){
  ...
  ws.onclose   = () => {
    wsReady = false;
    if (gameActive) setTimeout(connectWS, 2000);
  };
  ws.onerror = () => {};
}
...
function showResults(msg, won, isDraw){
  ...
  if (isDraw){
    if (tag)    { tag.textContent='DRAW';   tag.style.cssText='background:rgba(100,116,139,0.2);color:#94a3b8;'+tag.style.cssText; }
...
const FAKE_MSGS   = ['Give 500x','good luck!','lets gooo 🎮','gg','who wants to play?',
                     'nice shot!','my bubbles my rules','cant stop 🔥','bet it all','I\'m on a streak'];
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // make sure lobby is active screen
  document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));
