...
function placeBubble(r, c){
  ...
  if(pts > 0){
    ...
    if(canvas){
      const cx = canvas.width/2, cy = canvas.height-55;
      const comboText = comboCount >= 3 ? ' 🔥COMBO x'+comboCount : '';
      scorePopups.push({x:cx, y:cy-20, text:'+'+pts+comboText, alpha:1, vy:-2.5, color: pts>=5?'#ffd700':'#22c55e'});
    }
    screenFlash = 8;
    const msgEl = document.getElementById('msg');
    if(msgEl){
      msgEl.textContent = pts >= 5 ? '🔥 COMBO! +'+pts : '✨ +'+pts;
      msgEl.style.color = pts >= 5 ? '#ffd700' : '#22c55e';
  ...
  // No individual win - game ends when timer runs out
  // Just check if grid is empty to notify (server handles win condition)
  let rem = 0;
  for(let r=0;r<GRID_ROWS;r++) for(let c=0;c<GRID_COLS;c++) if(grid[r][c]) rem++;
  if(rem === 0 && typeof wsSend==='function'){
    // Refill grid so game continues
    for(let r=0;r<3;r++) for(let c=0;c<GRID_COLS;c++)
...
function gameLoop(){
  ...
  if(oppIsShooting){
    ...
    // Same collision detection as player bullet
    let oppHit = false;
